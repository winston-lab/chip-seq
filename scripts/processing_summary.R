...
main = function(in_table, surv_abs_out, surv_rel_out, loss_out){
    ...
    ggsave(surv_abs_out, plot=surv_abs, width=20, height=2+2.5*nsamples, units="cm", limitsize=FALSE)
    ggsave(surv_rel_out, plot=surv_rel, width=20, height=2+2.5*nsamples, units="cm", limitsize=FALSE)
    ...
    ggsave(loss_out, plot=loss_plot, width=20, height=2+1.5*nsamples, units="cm", limitsize=FALSE)
}
