...
wildcard_constraints:
    ...
    readtype = "|".join(["plus", "minus", "midpoints", "protection", f"ratio_window_{config['coverage_binsize']}"]),
    windowsize = "\d+",
...
rule target:
    input:
        ...
        expand(f"coverage/{{norm}}/{{sample}}_{FACTOR}-chipseq-{{norm}}-{{readtype}}.bw",
                ...
                sample=SISAMPLES,
                readtype=["plus", "minus", "protection", "midpoints", "midpoints_smoothed"]),
        expand(f"coverage/libsizenorm/{{group}}_{FACTOR}-chipseq-libsizenorm-ratio_window_{{windowsize}}.bw",
                group=validgroups,
                windowsize=config["coverage_binsize"]),
        expand(f"coverage/spikenorm/{{group}}_{FACTOR}-chipseq-spikenorm-ratio_window_{{windowsize}}.bw",
                group=validgroups_si,
                windowsize=config["coverage_binsize"]),
        #scatterplots
