...
rule target:
    input:
        ...
        # expand(expand("datavis/{{figure}}/libsizenorm/{condition}-v-{control}/passing/{{readtype}}/{{factor}}-chipseq_{{figure}}-libsizenorm-passing_{condition}-v-{control}_{{readtype}}-heatmap-bysample.svg",
        #               zip,
        #               condition=conditioncheck(conditiongroups),
        #               control=conditioncheck(controlgroups)),
        #        figure=FIGURES,
        #        readtype=f"ratio_window_{config['coverage_binsize']}",
        #        factor=FACTOR) if comparisons and config["plot_figures"] else [],
        ...
        # expand(expand("datavis/{{figure}}/spikenorm/{condition}-v-{control}/passing/{{readtype}}/{{factor}}-chipseq_{{figure}}-spikenorm-passing_{condition}-v-{control}_{{readtype}}-heatmap-bysample.svg",
        #               zip,
        #               condition=conditioncheck(conditiongroups_si),
        #               control=conditioncheck(controlgroups_si)),
        #        figure=FIGURES,
        #        readtype=f"ratio_window_{config['coverage_binsize']}",
        #        factor=FACTOR) if comparisons_si and config["plot_figures"] else [],
        ...
        expand(expand("diff_binding/{{annotation}}/{condition}-v-{control}/libsizenorm/{condition}-v-{control}_{{factor}}-chipseq-libsizenorm-{{annotation}}-diffbind-results-{{direction}}.tsv",
                      zip,
        ...
        expand(expand("diff_binding/{{annotation}}/{condition}-v-{control}/spikenorm/{condition}-v-{control}_{{factor}}-chipseq-spikenorm-{{annotation}}-diffbind-results-{{direction}}.tsv",
                       zip,
